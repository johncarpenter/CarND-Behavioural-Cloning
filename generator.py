...
class DriveImageDataGenerator(object):
    ...
    def flow(self, X, y=None, batch_size=32, shuffle=True, seed=None,
        ...
        return DriveNumpyArrayIterator(
            X, y, self,
...
class DriveNumpyArrayIterator(Iterator):
    ...
    def __init__(self, x, y, image_data_generator,
        ...
        self.x = np.asarray(x)
        if self.x.ndim != 4:
            raise ValueError('Input data in `DriveNumpyArrayIterator` '
                             'should have rank 4. You passed an array '
                             'with shape', self.x.shape)
        channels_axis = 3 if dim_ordering == 'tf' else 1
        if self.x.shape[channels_axis] not in {1, 3, 4}:
            raise ValueError('DriveNumpyArrayIterator is set to use the '
        ...
        super(DriveNumpyArrayIterator, self).__init__(x.shape[0], batch_size, shuffle, seed)
